...
- (void)viewDidLoad
{
 ...
 //Comment this out if using FRC
    ...
    self.cancelButton = [[UIBarButtonItem alloc] initWithTitle:@"CANCEL" style:UIBarButtonItemStylePlain target:self action:@selector(editTableView)];
...
//    [toolBarButtonsArray addObject:self.exportButton];
    ...
    self.arrayStatusLabel.font = [self.themeSettings fontWithName:@"Raleway-Regular" regularSize:21 bigSize:23 biggest:25];
...
-(void) newExport
{
    ...
    UIAlertAction * textAction = [UIAlertAction actionWithTitle:@"Text" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        {
            UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"Oops!" message:@"Can't send text message.  Try removing a file or two." delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil] ;
            [alert show];
        }
        ...
        if ([MFMailComposeViewController canSendMail])
        {
            ...
            if (sendSpecificRows)
            {
                for (NSIndexPath *selectionIndex in selectedRows)
                {
                    Recording * recording = [self.fetchCon objectAtIndexPath:selectionIndex];
                    NSURL * audioURL = [NSURL URLWithString:[recording.audioURL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
//                    [self sendMailPicker:controller attachmentURL:audioURL andTitle:[recording.audioURL lastPathComponent]];
        ...
        {
            UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"Oops!" message:@"Can't send mail" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil] ;
            [alert show];
        }
